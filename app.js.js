(function () {
  ...
  function buildGpgl(rects, moduleMm, stepsPerMm, gridSize) {
    ...
    for (const rect of rects) {
      ...
      // One coordinate pair per PU/PD command -- some plotter firmware
      // does not reliably handle multi-point polyline continuations
      // chained onto a single PD, so each segment gets its own command.
      lines.push(`PU${x1},${yBottom};`);
      lines.push(`PD${x1},${yTop};`);
      lines.push(`PD${x2},${yTop};`);
      lines.push(`PD${x2},${yBottom};`);
      lines.push(`PD${x1},${yBottom};`);
    }
