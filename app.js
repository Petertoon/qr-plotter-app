(function(){
  ...
  const els={
    text:$('qr-text'),targetSize:$('target-size'),targetUnit:$('target-unit'),
    ec:$('ec-level'),steps:$('steps-per-mm'),quiet:$('quiet-zone'),
    mediaWidth:$('media-width'),mediaUnit:$('media-unit'),margin:$('margin'),
    generate:$('generate-btn'),test:$('test-btn'),canvas:$('qr-canvas'),
    stats:$('stats'),warning:$('warning'),download:$('download-area')
  };
  ...
  function generateQR(){
    ...
    const quiet=Math.max(0,parseInt(els.quiet.value,10)||0);
    ...
    els.stats.textContent=
      ...
      `finished size: ${finished.toFixed(1)} mm `+
      `(${(finished/25.4).toFixed(2)} in)\n`+
